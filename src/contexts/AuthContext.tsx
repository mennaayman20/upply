...
interface AuthContextType {
  ...
  signup: (email: string, password: string, role: UserRole) => Promise<{ user: User; otp: string }>;
...
export const AuthProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const signup = async (email: string, password: string, role: UserRole): Promise<{ user: User; otp: string }> => {
    return mockAuth.signup(email, password, role);
  };

  const verifyOTP = async (otp: string): Promise<boolean> => {
    const user = mockAuth.verifyOTP(otp);
    if (user) {
      setUser(user);
      return true;
    }
